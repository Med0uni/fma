...
export default function NewsCard({
  ...
  return (
    <motion.div whileHover={{ scale: 1.01 }} transition={{ duration: 0.2 }}>
      <Link href={`/news/${slug}`}>
        <article className="group relative overflow-hidden rounded-lg border bg-card transition-all duration-300 hover:border-primary/50 hover:shadow-lg">
          <div className="flex h-full flex-col md:flex-row">
            ...
            <div className="flex-1 p-4 md:p-6">
              ...
              <div className="flex items-center text-sm text-primary transition-colors group-hover:text-primary/80 md:text-base">
                <span>{readMoreText}</span>
                <ChevronRight
                  className={`h-4 w-4 transition-transform group-hover:translate-x-1 ${language === 'ar' ? 'rotate-180' : ''}`}
                />
              </div>
            </div>
          </div>
